...
#define KCYN  "\x1B[36m"
#define KWHT  "\x1B[37m"
#define SIZE 3 //or 80 or 5 (matrix, smatrix)
...
int main(){
	...
	FILE *data;
	data = fopen("Euler_81_ssmatrix.txt", "r"); // read only, smatrix(5x5) or matrix(80x80)
  ...
  //	for (int j = SIZE - 2; j >= 1; j-- ){
  		score[i][j] += MIN3(score[i+1][j], score[i][j+1], score[i-1][j]); //score for given entry is itself + min[entry below , entry to right, entry above]
  	}
  }
